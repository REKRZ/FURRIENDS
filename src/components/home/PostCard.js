      ...
      <article className='overflow-hidden rounded-lg shadow-2xl'>
        <a className='w-full h-full' href='#'>
          <label htmlFor={`post-modal-${i}`} className='modal-button'>
            <img className='block object-scale-down h-80 w-full' src={post.uploadedPhoto} alt='pic' />
          </label>
        </a>
        {/* start of modal here */}
        <input type='checkbox' id={`post-modal-${i}`} className='modal-toggle' />
          ...
          <label className='modal-box relative' htmlFor=''>
            <div key={i} className='flex place-content-center my-1 px-1 w-full'>
              <article className='overflow-hidden rounded-lg shadow-lg'>
                <a className='w-full h-full' href='#'>
                  <img className='block object-scale-down h-200 w-full' src={post.uploadedPhoto} alt='pic' />
                </a>
        ...
        <header className='flex items-center leading-none p-2 md:p-4 h-48'>
          <h1 className='w-3/4 text-lg justify-self-start ml-2'>{post.caption}</h1>
          <p className='w-1/4 text-grey-darker text-xs place-self-end pl-2'>{post.createdAt.toDate().toDateString().split(' ').slice(1).join(' ')}</p>
        </header>
        <footer className='flex items-center justify-between leading-none p-2 md:p-4'>
          <div className='flex items-center no-underline hover:underline'>
            <img alt='' className='block rounded-full h-7 w-7 mr-1' src={post.profilePic} />
            {post.uid !== uid ? (
              <Link to='/friendprofile' className='ml-2 text-sm' state={{ from: post.uid }}>
                {post.displayName}
              </Link>
            ) : (
          ...
          <a className='flex place-items-center no-underline text-grey-darker hover:text-red-dark' href='#'>
